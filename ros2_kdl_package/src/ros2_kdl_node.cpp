...
public:
	...
	prev_dqd.data.setZero(); // Inizializzazione a zero
...
private:
    void cmd_publisher()
    {
        ...
        if (t_ <= total_time)
        {
            ...
            else if (cmd_interface_ == "effort" && control_space_ == "operational_space")
            {
            ...
            else if (cmd_interface_ == "effort" && control_space_ == "joint_space")
            {
            	//Effort control using idCntr_JSpace
            	
	    	//Calcolo della posizione, velocità e accelerazione desiderata dei giunti
	    	KDL::JntArray qd(joint_positions_.data.size());
	    	KDL::JntArray dqd(joint_positions_.data.size());
	    	KDL::JntArray ddqd(joint_positions_.data.size());
